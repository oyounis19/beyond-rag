...
from ..models.app_models import Conflict, Chunk, DedupGroup, DedupGroupMember
...
@router.get("/dedup")
def list_dedup_groups(db: Session = Depends(get_db)):
    ctx = AppContext()
    s = ctx.get_db_session()
    try:
        groups = s.query(DedupGroup).limit(100).all()
        out = []
        for g in groups:
            count = s.query(DedupGroupMember).filter(DedupGroupMember.group_id == g.id).count()
            out.append({"group_id": str(g.id), "representative_chunk_id": str(g.representative_chunk_id) if g.representative_chunk_id else None, "members": count})
        return out
    finally:
        ctx.close_session(s)
